...
public class Simulator {
	...
	public void simulate() {
		int duration;
		boolean carArrived, carLeaving, isParked;
		Car car;
		Spot priority = null;
		Spot incomingCar = null;
		Car priorityCar = null;
		// Local variables can be defined here.
		int cols = lot.getNumSpotsPerRow();
		int rows = lot.getNumRows();
		this.clock = 0;
		// Note that for the specific purposes of A2, clock could have been 
		// defined as a local variable too.
		
		while (clock < steps) {			
			//Probability car gets queued into parking lot
			carArrived = RandomGenerator.eventOccurred(probabilityOfArrivalPerSec);
			if (carArrived){
				car = RandomGenerator.generateRandomCar();
				incomingQueue.enqueue(new Spot(car, clock));
			}
			
			//Probability car gets removed
			for (int i = 0; i<rows; i++){
				for (int j = 0; j< cols; j++){
					Spot parkingSpot = lot.getSpotAt(i, j);
					if (parkingSpot !=null){
						duration = (clock - parkingSpot.getTimestamp());
					}
					else{
						duration = 0;
					}
					if (duration == 3600*8){
						Spot leave = lot.remove(i, j);
						outgoingQueue.enqueue(leave);
					}
					else{
						carLeaving = RandomGenerator.eventOccurred(departurePDF.pdf(duration));
						if (carLeaving){
							Spot leave = lot.remove(i,j);
							outgoingQueue.enqueue(leave);
						}
			...
			//incoming queue remover
			if (!incomingQueue.isEmpty()){
				if (priority == null){
					priority = incomingQueue.dequeue();
					priorityCar = priority.getCar();
				}
				else{
					if (lot.attemptParking(priorityCar, clock) == true){

						System.out.println(priorityCar + " ENTERED at timestep " + clock + "; occupancy is at " +lot.getTotalOccupancy());
						priority = null;
						priorityCar = null;
					}
				}
			}
			//outgoing queue remover
			if (!outgoingQueue.isEmpty()){
				Spot c = outgoingQueue.dequeue();
				Car d = c.getCar();
				System.out.println(d + " EXITED at timestep " + clock + "; occupancy is at " +lot.getTotalOccupancy());
			}
			clock++;
		}
	}
