...
public class ParkingLot {
	...
	public void park(int i, int j, Car c, int timestamp) {
		...
		if (!canParkAt(i, j, c)){
			System.out.print("Car cannot be parked at this location");
			return;
		}
	...
	public boolean canParkAt(int i, int j, Car c) {
		...
		if (carType == CarType.ELECTRIC){
			return (spotType == CarType.LARGE || spotType == CarType.REGULAR || spotType == CarType.SMALL || spotType==CarType.ELECTRIC);
		}
		else if (carType == CarType.SMALL){
			return (spotType == CarType.LARGE || spotType == CarType.REGULAR || spotType == CarType.SMALL);
		}
		else if (carType == CarType.REGULAR){
			return (spotType == CarType.LARGE || spotType == CarType.REGULAR);
		}
		...
		while (scanner.hasNext()){
			...
			if (str.isEmpty()){
				...
				for (int j = 0; j<line.length; j++){
					lotDesign[rowIndex][j] = Util.getCarTypeByLabel(line[j].trim());
				}
				rowIndex++;
			}
		}
